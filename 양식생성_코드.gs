 ...
 *
 * 학생이 2단계에서 «프롬프트 복사» 를 누른 뒤 제미나이를 거치지 않고
 * 곧바로 3단계에 붙여넣는 실수가 잦다. 그러면 프롬프트 안의 예시 껍데기로
 * « 1. 첫 번째 항목 제목 » 같은 빈 문서가 만들어져 학생이 헷갈린다.
 *
 * 프롬프트에만 있는 두 가지가 함께 보일 때만 막는다.
 *   서두 — 프롬프트를 여는 말투와 규칙 문구
 *   예시 — 문서 틀 설명에 쓰인 자리표시 낱말
 * 둘 다 있어야 막으므로, 진짜 답변을 잘못 막을 일은 거의 없다.
 */
function 프롬프트인가_(전체, 틀) {
  var 원문 = String(전체 || '');
  var 표본 = (틀 ? 틀.join('\n') : '') + '\n' + 원문;
  var 예시 = /첫 번째 항목 제목|내가 답해야 할 질문|머리글1|머리글2/.test(표본);
  var 서두 = /너는 고등학교 3학년|맨 끝에 \[문서 틀\] 을 만들어 줘|지켜야 할 규칙/.test(원문);
  return 예시 && 서두;
}
