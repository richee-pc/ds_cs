...
var VER = 18;
var 만든판 = 'v18 · 2026-08-21';   // 결과물에 찍어 둔다. 어느 판으로 만든 파일인지 한눈에 알려고.
...
function makePrompt(info) {
  try {
    ...
    if (String(info.task) === '2') {
      ...
      줄.push('');
      줄.push('=== 실습 데이터 시작 ===');
      줄.push('data = ["데미안", "토지", "모비딕", "광장", "이방인", "삼국지", "파우스트",');
      줄.push('        "페스트", "죄와 벌", "동물농장", "어린 왕자", "노인과 바다",');
      줄.push('        "위대한 개츠비", "백년의 고독", "호밀밭의 파수꾼", "자기 앞의 생",');
      줄.push('        "수레바퀴 아래서", "싯다르타", "1984", "설국"]');
      줄.push('target = "싯다르타"');
      ...
      줄.push('');
      줄.push('  실습 데이터 규칙');
      줄.push('  1. data 는 20개를 «하나도 빠짐없이 실제 값으로» 적어.');
      줄.push("     '...' 이나 '(생략)' 같은 줄임표를 절대 쓰지 마. 그러면 코드가 안 돌아가.");
      줄.push('  2. 20개는 모두 같은 종류로. 글자면 전부 따옴표, 숫자면 전부 따옴표 없이.');
