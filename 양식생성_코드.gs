...
var VER = 19;
var 만든판 = 'v19 · 2026-08-21';   // 결과물에 찍어 둔다. 어느 판으로 만든 파일인지 한눈에 알려고.
...
function makePrompt(info) {
  try {
    ...
    // 자세한 형식은 맨 끝 «답변 형식» 에 한 번만 적는다.
    // 앞뒤로 흩어 두면 제미나이가 맨 끝 지시만 따르고 실습 데이터를 빠뜨린다.
    if (String(info.task) === '2') {
      줄.push('이 보고서에는 코드에 넣어 돌릴 [실습 데이터] 가 반드시 필요해.');
      줄.push('맨 끝 «답변 형식» 에 적은 대로 답안보다 먼저 줘.');
      줄.push('');
    }
    ...
    if (String(info.task) === '2') {
      ...
      var 보기 = 보기자료_(개수);
      var 줄바꿈넣기 = [];
      for (var q = 0; q < 보기.length; q += 4) {
        줄바꿈넣기.push('        "' + 보기.slice(q, q + 4).join('", "') + '"'
          + (q + 4 < 보기.length ? ',' : ']'));
      }

      줄.push('답변은 아래 두 덩어리로 줘. 실습 데이터를 먼저, 답안을 그다음에.');
      줄.push('');
      줄.push('=== 실습 데이터 시작 ===');
      줄.push('data = [');
      줄바꿈넣기.forEach(function (l) { 줄.push(l); });
      줄.push('target = "' + 보기[Math.floor(개수 / 2)] + '"');
      줄.push('# 없는 값 : "해리 포터와 마법사의 돌"');
      줄.push('=== 실습 데이터 끝 ===');
      줄.push('');
      줄.push('  위는 «학교 도서관 도서 검색» 일 때의 보기야. 내 주제에 맞게 새로 만들어 줘.');
      줄.push('');
      줄.push('  실습 데이터 규칙');
      줄.push('  1. data 는 정확히 ' + 개수 + '개. «하나도 빠짐없이 실제 값으로» 적어.');
      줄.push("     '...' 이나 '(생략)' 같은 줄임표를 절대 쓰지 마. 그러면 코드가 안 돌아가.");
      // 이진 탐색 최악 단계 수는 floor(log2 n) + 1 이다.
      // ceil(log2 n) 로 하면 32 처럼 2의 거듭제곱에서 한 단계 적게 나온다.
      var 단계 = Math.floor(Math.log(개수) / Math.LN2) + 1;
      var 규모말 = (info.size || '').trim();
      규모말 = 규모말 ? (규모말 + '이라서') : '이 정도 규모라서';
      줄.push('     (내 자료가 ' + 규모말 + ' ' + 개수 + '개로 줄여 손으로 따라가 볼 거야.');
      줄.push('      이진 탐색이 많아야 ' + 단계
        + '단계라서 «탐색 과정 표» 6줄에 알맞게 들어가.)');
      줄.push('  2. ' + 개수 + '개는 모두 같은 종류로. 글자면 전부 따옴표, 숫자면 전부 따옴표 없이.');
      줄.push('     (섞이면 코드가 오류를 냅니다)');
      줄.push('  3. 내 주제에 실제로 있을 법한 값으로. 겹치는 값은 없게.');
      줄.push('  4. target 은 data 안에 실제로 있는 값 하나. 맨 앞·맨 뒤 말고 가운데쯤.');
      줄.push("  5. '없는 값' 은 data 에 절대 없는 값으로.");
      줄.push('  6. data 를 정렬하지 마. 코드가 알아서 정렬해.');
      줄.push('');
      줄.push('그다음 답안을 아래 표시 사이에 넣어 줘.');
    } else {
...
function 웹문서HTML_(spec, 이름, info, 모양, 조각) {
  ...
  h.push(' .draft{position:relative}');
  h.push(' .copy{position:absolute;top:9px;right:10px;font-size:.72em;padding:5px 10px;');
  h.push('   background:#fff;color:var(--e);border:1px solid var(--e);border-radius:5px;cursor:pointer;font-weight:700}');
  ...
  h.push(' textarea{width:100%;min-height:130px;overflow:hidden;font-family:inherit;font-size:.95em;line-height:1.8;');
  ...
  if (항목들 && 항목들.length) {
    항목들.forEach(function (it, n) {
      ...
      if (it.초안.length) {
        ...
        h.push('<button class="copy" data-for="mine' + n + '">초안 복사</button>');
        h.push('</div>');
      }
      if (it.점검.length) {
  ...
  if (실측있음) {
    h.push(코드보이기_());
    h.push(시뮬레이터_(목록, 실습데이터뽑기_(조각, 데이터개수_(info))));
  }

  // ---- 맨 아래 ----
  h.push('<div class="bar">');
  h.push('<button id="submit">제출 파일 만들기</button>');
  h.push('<button class="ghost" id="resume">이어 쓰기 — 저장한 제출 파일 불러오기</button>');
  h.push('<input type="file" id="resume-f" accept=".html,.htm,text/html" style="display:none">');
  ...
  h.push('<p class="note" style="text-align:center;margin-top:6px">');
  h.push('집에서 이어 쓰려면 <b>제출 파일을 만들어 두었다가</b>, 새 파일을 연 뒤 ');
  h.push('<b>«이어 쓰기»</b> 로 그 파일을 불러오세요.</p>');
...
function 실습데이터뽑기_(조각, 바람) {
  ...
  // 표시가 있으면 그 안에서만 찾는다. 답안 본문의 예시를 잘못 집지 않으려는 것이다.
  var 표 = /===\s*실습 데이터 시작\s*===([\s\S]*?)===\s*실습 데이터 끝\s*===/.exec(전체);
  if (표) 전체 = 표[1];
  ...
  // 제미나이가 «...» 처럼 줄여 쓰면 값이 모자란다. 그 사실을 학생에게 알린다.
  var 줄임 = /\.\.\.|…|생략/.test(m[1]);
  var 적음 = 바람 ? (값.length < 바람 - 2) : (값.length < 10);
  return { data: 값, target: 찾을값, 없는값: 없는값, 받음: true,
           바람: 바람 || 0, 모자람: (줄임 || 적음) ? 값.length : 0 };
}
 ...
 * 실습 데이터를 몇 개 받을지 정한다.
 *
 * 근거는 «탐색 과정 표» 가 6줄이라는 것이다.
 * 이진 탐색 단계 수는 log2(n) 이므로 32개면 6단계로 표가 꽉 찬다.
 * 자료가 작다고 고른 학생에게 32개를 주면 앞뒤가 안 맞으니 규모에 맞춘다.
 */
function 데이터개수_(info) {
  var 규모 = String((info && info.size) || '').trim();
  if (규모.indexOf('수십만') >= 0) return 32;   // 이진 6단계
  if (규모.indexOf('수십') >= 0) return 12;     // 이진 4단계
  if (규모.indexOf('수천') >= 0) return 20;     // 이진 5단계
  return 20;                                    // 안 골랐으면 가운데로
}


/** 프롬프트에 넣을 보기 목록. 필요한 개수만큼 잘라 쓴다. */
function 보기자료_(개수) {
  var 책 = ['데미안', '토지', '모비딕', '광장', '이방인', '삼국지', '파우스트',
            '페스트', '죄와 벌', '동물농장', '어린 왕자', '노인과 바다',
            '위대한 개츠비', '백년의 고독', '호밀밭의 파수꾼', '자기 앞의 생',
            '수레바퀴 아래서', '싯다르타', '1984', '설국',
            '변신', '고도를 기다리며', '아Q정전', '홍길동전', '무정', '날개',
            '메밀꽃 필 무렵', '운수 좋은 날', '사랑손님과 어머니', '봄봄',
            '벙어리 삼룡이', '태백산맥'];
  return 책.slice(0, 개수);
}
...
function 시뮬레이터_(목록, 실습) {
  ...
  if (D.받음) {
    ...
    if (D.모자람) {
      h.push('<p class="note" style="color:#b45309"><b>다만 ' + D.모자람 + '개뿐입니다.</b> ');
      if (D.바람) h.push('(' + D.바람 + '개를 받기로 했습니다) ');
      h.push('제미나이가 «...» 처럼 줄여 썼을 수 있습니다. ');
      h.push('20개가 되도록 <b>위 칸에 직접 채워 넣거나</b>, 제미나이에게 ');
      h.push('«줄임표 없이 20개를 다 적어 줘» 라고 다시 물어보세요.</p>');
    }
  } else {
...
function 저장스크립트_() {
  ...
  j.push('  불러오기();');
  j.push('  // 없는 단추에 붙이려다 터지면 그 아래 줄이 전부 죽는다.');
  j.push('  // 수행평가 1 처럼 «실행 도구» 가 없는 결과물도 있으므로 반드시 확인하고 붙인다.');
  j.push('  function 눌리면(id, 할일){');
  j.push('    var e = document.getElementById(id);');
  j.push('    if (e) e.addEventListener("click", 할일);');
  j.push('    return e; }');
  j.push('');
  ...
  j.push('');
  j.push('  // 숫자 자료를 글자처럼 늘어놓으면 "10057" 이 "1043" 앞에 온다.');
  j.push('  // 값이 모두 숫자면 숫자로 견준다.');
  j.push('  function 다숫자(a){ return a.every(function(x){ return /^-?\\d+(\\.\\d+)?$/.test(x); }); }');
  j.push('  function 줄세우기(a){ return 다숫자(a)');
  j.push('    ? a.slice().sort(function(p,q){ return p-q; }) : a.slice().sort(); }');
  j.push('  function 앞선가(p, q, 숫자){ return 숫자 ? (p-0) < (q-0) : p < q; }');
  j.push('');
  j.push('  function 이진(a, t){ var 숫 = 다숫자(a.concat([t]));');
  j.push('    var b=줄세우기(a), lo=0, hi=b.length-1, n=0, 줄=[];');
  ...
  j.push('      if(앞선가(b[m], t, 숫)){ 줄.push(말+"  -> 오른쪽 절반만 남김"); lo=m+1; }');
  ...
  j.push('');
  j.push('  눌리면("run", function(){ 돌리기(document.getElementById("d-t").value.trim()); });');
  j.push('  눌리면("runx", function(){ 돌리기(document.getElementById("d-x").value.trim()); });');
  j.push('');
  j.push('  // 이진 탐색이 가장 여러 단계 걸리는 값을 찾아 준다.');
  j.push('  // 가운데에 딱 걸리는 값을 고르면 한 번에 끝나 표가 한 줄뿐이라 근거가 약하다.');
  j.push('  눌리면("runw", function(){');
  j.push('    var a=값들(); if(a.length<2){ alert("data 를 두 개 이상 넣어 주세요."); return; }');
  j.push('    var 숫=다숫자(a), b=줄세우기(a), 고른값=b[0], 최대=0;');
  j.push('    b.forEach(function(v){ var lo=0,hi=b.length-1,n=0;');
  j.push('      while(lo<=hi){ var m=Math.floor((lo+hi)/2); n++;');
  j.push('        if(b[m]===v) break; else if(앞선가(b[m], v, 숫)) lo=m+1; else hi=m-1; }');
  j.push('      if(n>최대){ 최대=n; 고른값=v; } });');
  j.push('    document.getElementById("d-t").value=고른값; 돌리기(고른값); });');
  j.push('');
  j.push('  눌리면("bench", function(){');
  ...
  j.push('  });');
  j.push('');
  j.push('  // ---- 칸이 글에 맞춰 늘어난다. 인쇄할 때 잘리지 않게 하려는 것이다. ----');
  j.push('  function 키우기(t){ t.style.height="auto"; t.style.height=(t.scrollHeight+4)+"px"; }');
  j.push('  function 모두키우기(){ 칸.forEach(키우기); }');
  j.push('  칸.forEach(function(t){ t.addEventListener("input", function(){ 키우기(t); }); });');
  j.push('  모두키우기();');
  j.push('  window.addEventListener("beforeprint", 모두키우기);');
  j.push('');
  j.push('  // ---- AI 초안 복사 ----');
  j.push('  function 복사하기(글, 단추){');
  j.push('    var 본디 = 단추.textContent;');
  j.push('    function 됐다(){ 단추.textContent = "복사됨!";');
  j.push('      setTimeout(function(){ 단추.textContent = 본디; }, 1200); }');
  j.push('    function 옛방식(){');
  j.push('      var t = document.createElement("textarea");');
  j.push('      t.value = 글; t.style.position="fixed"; t.style.opacity="0";');
  j.push('      document.body.appendChild(t); t.select();');
  j.push('      try { document.execCommand("copy"); 됐다(); }');
  j.push('      catch(e){ 단추.textContent = "복사 안 됨"; ');
  j.push('        setTimeout(function(){ 단추.textContent = 본디; }, 1500); }');
  j.push('      t.remove(); }');
  j.push('    if (navigator.clipboard && navigator.clipboard.writeText) {');
  j.push('      navigator.clipboard.writeText(글).then(됐다, 옛방식);');
  j.push('    } else { 옛방식(); } }');
  j.push('');
  j.push('  [].slice.call(document.querySelectorAll(".copy")).forEach(function(b){');
  j.push('    b.addEventListener("click", function(){');
  j.push('      var 상자 = b.parentNode;');
  j.push('      var 글 = [].slice.call(상자.querySelectorAll("p"))');
  j.push('        .map(function(p){ return p.textContent; }).join("\\n");');
  j.push('      복사하기(글, b); }); });');
  j.push('');
  j.push('  // ---- 이어 쓰기 : 제출 파일에서 내가 쓴 글을 되살린다 ----');
  j.push('  var 고르기 = document.getElementById("resume-f");');
  j.push('  눌리면("resume", function(){ if (고르기) 고르기.click(); });');
  j.push('  if (고르기) 고르기.addEventListener("change", function(){');
  j.push('    var f = 고르기.files && 고르기.files[0];');
  j.push('    if(!f) return;');
  j.push('    var r = new FileReader();');
  j.push('    r.onload = function(){');
  j.push('      var 자료 = null;');
  j.push('      try {');
  j.push('        var m = String(r.result).match(/id="채점자료">([\\s\\S]*?)<\\/scr' + 'ipt>/);');
  j.push('        if (m) 자료 = JSON.parse(m[1].replace(/\\\\u003c/g, "<"));');
  j.push('      } catch(e){}');
  j.push('      if(!자료 || !자료.항목){');
  j.push('        alert("이 파일에서는 쓴 글을 찾지 못했습니다.\\n\\n"');
  j.push('          + "«제출 파일 만들기» 로 만든 _제출.html 파일을 골라 주세요.");');
  j.push('        고르기.value=""; return; }');
  j.push('      var 원본 = JSON.parse(document.getElementById("원본자료").textContent);');
  j.push('      if(자료.이름표 && 원본.이름표 && 자료.이름표 !== 원본.이름표){');
  j.push('        if(!confirm("다른 수행평가의 파일입니다.\\n\\n"');
  j.push('          + "이 파일: " + 원본.이름표 + "\\n고른 파일: " + 자료.이름표');
  j.push('          + "\\n\\n그래도 불러올까요?")){ 고르기.value=""; return; } }');
  j.push('      var 쓴게있나 = 칸.some(function(t){ return t.value.trim(); });');
  j.push('      if(쓴게있나 && !confirm("지금 쓴 글이 지워지고 불러온 글로 바뀝니다.\\n\\n괜찮습니까?")){');
  j.push('        고르기.value=""; return; }');
  j.push('      // 항목 제목으로 맞춘다. 못 맞추면 순서대로.');
  j.push('      var 됨 = 0;');
  j.push('      자료.항목.forEach(function(it, n){');
  j.push('        var 자리 = null;');
  j.push('        원본.항목.forEach(function(o){ if(o.제목 === it.제목) 자리 = o.id; });');
  j.push('        if(!자리 && 원본.항목[n]) 자리 = 원본.항목[n].id;');
  j.push('        var t = 자리 && document.getElementById(자리);');
  j.push('        if(t && it.내글){ t.value = it.내글; 됨++; } });');
  j.push('      if(자료.실행기록 && 자료.실행기록.length) 실행기록 = 자료.실행기록.slice();');
  j.push('      저장(); 모두키우기();');
  j.push('      alert("불러왔습니다.\\n\\n항목 " + 됨 + "개"');
  j.push('        + (자료.낸때 ? "  (" + 자료.낸때 + " 에 만든 파일)" : ""));');
  j.push('      고르기.value=""; };');
  j.push('    r.readAsText(f, "utf-8"); });');
  j.push('})();');
  j.push('<' + '/script>');
  return j.join('\n');
}
